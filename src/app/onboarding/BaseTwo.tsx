...
export default function BaseTwo() {
  ...
  useEffect(() => {
     AsyncStorage.setItem("name", user?.user_metadata?.name || "");
      ...
      <View className="flex-1">
          ...
          <View className="space-y-4">
            {chooseTitles.map((title, index) => (
              <Pressable
                ...
                className={`p-4 rounded-xl border-2 my-2 ${selectedIndex === index
                    ? "border-blue-500 "
                    : "border-gray-200 "
                  }`}
              >
                <View className="flex-row items-center justify-between">
                  <Text
                    className={`text-lg font-medium ${selectedIndex === index
                        ? "text-blue-600"
                        : "text-gray-700"
                      }`}
                  >
        ...
        <View className="p-5 my-10">
          <View className="flex-row space-x-4">
            <Pressable
              onPress={handleNext}
              className={`flex-1 py-5 px-6 rounded-full ${selectedIndex !== null ? "bg-black" : "bg-gray-300"
                }`}
              disabled={selectedIndex === null}
            >
              <Text
                className={`text-center font-medium ${selectedIndex !== null ? "text-white" : "text-gray-500"
                  }`}
              >
                Continue
              </Text>
