...
export default function Four() {
  ...
  // 检查是否所有选项都已选择
  const isAllSelected = selectedSkinTone && selectedBodyType && selectedBodyStructure && selectedFaceShape;
    ...
    <View className="flex-1 bg-white">
      ...
      <ScrollView>
        <View className="flex-1 justify-center px-5">
          ...
          {/* 肤色选择 */}
          <View className="mb-8">
            <Text className="text-lg font-semibold text-gray-800 mb-4">Skin Tone</Text>
            <View className="flex-row justify-between px-2">
              {SKIN_TONE_OPTIONS.map((option) => (
                <Pressable
                  key={option.id}
                  onPress={() => setSelectedSkinTone(option.id)}
                  className={`items-center p-2 rounded-xl border-2 flex-1 mx-1 ${selectedSkinTone === option.id
                ...
                >
                  <View
                    style={{
                      ...
                      borderRadius: 16,
                      borderWidth: 2,
                      borderColor: selectedSkinTone === option.id ? '#3b82f6' : '#e5e7eb',
                    }}
                  />
                </Pressable>
              ))}
            ...
            {selectedSkinTone && (
              <Text className="text-sm mt-2 text-center text-blue-600 font-medium">
                Selected: {SKIN_TONE_OPTIONS.find(opt => opt.id === selectedSkinTone)?.label}
              </Text>
            )}
          </View>
          ...
          {/* 体型选择 */}
          <View className="mb-8">
            <Text className="text-lg font-semibold text-gray-800 mb-4">Body Type</Text>
            <View className="flex-row flex-wrap px-2">
              {BODY_TYPE_OPTIONS.map((option, index) => (
                ...
                >
                  ...
                  <Text className={`text-xs mt-1 font-medium text-center ${selectedBodyType === option.id ? 'text-blue-600' : 'text-gray-700'
                    }`}>
                    {option.label}
                  </Text>
                </Pressable>
            ...
            {selectedBodyType && (
              <Text className="text-sm mt-2 text-center text-blue-600 font-medium">
                Selected: {BODY_TYPE_OPTIONS.find(opt => opt.id === selectedBodyType)?.label}
              </Text>
            )}
          </View>
          ...
          {/* 身体结构选择 */}
          <View className="mb-8">
            <Text className="text-lg font-semibold text-gray-800 mb-4">Body Structure</Text>
            <View className="flex-row flex-wrap px-2">
              {BODY_STRUCTURE_OPTIONS.map((option, index) => (
                <Pressable
                  key={option.id}
                  onPress={() => setSelectedBodyStructure(option.id)}
                  className={`items-center p-2 rounded-xl border-2 w-[30%] mb-3 ${selectedBodyStructure === option.id
                    ? 'border-blue-500 bg-blue-50'
                    : 'border-gray-200 bg-white'
                    }`}
                  style={{
                ...
                >
                  ...
                  <Text className={`text-xs mt-1 font-medium text-center ${selectedBodyStructure === option.id ? 'text-blue-600' : 'text-gray-700'
                    }`}>
                    {option.label}
                  </Text>
                </Pressable>
            ...
            {selectedBodyStructure && (
              <Text className="text-sm mt-2 text-center text-blue-600 font-medium">
                Selected: {BODY_STRUCTURE_OPTIONS.find(opt => opt.id === selectedBodyStructure)?.label}
              </Text>
            )}
          </View>
          ...
          {/* 脸型选择 */}
          <View className="mb-8">
            <Text className="text-lg font-semibold text-gray-800 mb-4">Face Shape</Text>
            <View className="flex-row justify-between px-2">
              {FACE_SHAPE_OPTIONS.map((option) => (
                <Pressable
                  key={option.id}
                  onPress={() => setSelectedFaceShape(option.id)}
                  className={`items-center p-2 rounded-xl border-2 flex-1 mx-1 ${selectedFaceShape === option.id
                    ? 'border-blue-500 bg-blue-50'
                    : 'border-gray-200 bg-white'
                    }`}
                >
                  <Text className="text-xl">{option.icon}</Text>
                  <Text className={`text-xs mt-1 font-medium text-center ${selectedFaceShape === option.id ? 'text-blue-600' : 'text-gray-700'
                    }`}>
                    {option.label}
                  </Text>
                </Pressable>
            ...
            {selectedFaceShape && (
              <Text className="text-sm mt-2 text-center text-blue-600 font-medium">
                Selected: {FACE_SHAPE_OPTIONS.find(opt => opt.id === selectedFaceShape)?.label}
              </Text>
            )}
          </View>
          ...
          <View className="mb-6">
            <View className="flex-row justify-between items-center mb-2">
              ...
              <Text className="text-sm text-blue-600 font-medium">
                {[selectedSkinTone, selectedBodyType, selectedBodyStructure, selectedFaceShape].filter(Boolean).length}/4
              </Text>
            </View>
          ...
          <View className="p-5 mb-20">
            <View className="flex-row space-x-4">
              <Pressable
                onPress={handleNext}
                className={`flex-1 py-3 px-6 rounded-full ${isAllSelected ? 'bg-blue-500' : 'bg-gray-300'
                  }`}
                disabled={!isAllSelected}
              >
                <Text className={`text-center font-medium ${isAllSelected ? 'text-white' : 'text-gray-500'
                  }`}>
                  Continue
                </Text>
              </Pressable>
