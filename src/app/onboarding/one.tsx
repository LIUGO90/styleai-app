      ...
      <ScrollView>
            ...
            {chooseTitles.map((title, index) => (
              ...
              >
                <View className="flex-row items-center justify-between">
                  <Text className={`text-lg font-medium ${selectedIndex === index ? 'text-blue-600' : 'text-gray-700'
                    }`}>
                    {title}
                  </Text>
                  {selectedIndex === index && (
        ...
        <View className="p-5 my-10">
          <View className="flex-row space-x-4">
            ...
            <Pressable
              onPress={handleNext}
              className={`flex-1 py-5 px-6 rounded-full ${selectedIndex !== null
                ? 'bg-blue-500'
                : 'bg-gray-300'
                }`}
              disabled={selectedIndex === null}
            >
              <Text className={`text-center font-medium ${selectedIndex !== null ? 'text-white' : 'text-gray-500'
                }`}>
                Continue
              </Text>
            </Pressable>
