...
export class BackTaskManager {
    ...
    public defineImageUploadTask() {
        ...
        TaskManager.defineTask(taskName, async () => {
            try {
                ...
                // 更新任务状态
                await this.updateTaskStatus(taskName, BackgroundFetch.BackgroundFetchResult.NewData);

                console.log(`后台图片上传完成，成功上传 ${successCount} 张图片`);
                return BackgroundFetch.BackgroundFetchResult.NewData;
            } catch (error) {
                console.error('后台图片上传任务失败:', error);
                await this.updateTaskStatus(taskName, BackgroundFetch.BackgroundFetchResult.Failed);
        ...
        TaskManager.defineTask(taskName, async () => {
            try {
                ...
                if (shouldSendNotification) {
                    ...
                    console.log('无需发送通知');
                    await this.updateTaskStatus(taskName, BackgroundFetch.BackgroundFetchResult.NoData);
                    return BackgroundFetch.BackgroundFetchResult.NoData;
                }
            } catch (error) {
    ...
    private async registerTask(taskName: string, task: BackgroundTask) {
        try {
            ...
            // 注册后台获取任务
            await BackgroundFetch.registerTaskAsync(taskName, task.options || {
    ...
    // 更新任务状态
    private async updateTaskStatus(taskName: string, result: BackgroundFetch.BackgroundFetchResult) {
    ...
    private async saveTaskStatuses() {
        try {
            ...
            await AsyncStorage.setItem('backgroundTaskStatuses', JSON.stringify(statuses));
        } catch (error) {
    ...
    public async addPendingUploadImage(imageUri: string, metadata?: any) {
        try {
            ...
            await AsyncStorage.setItem('pendingUploadImages', JSON.stringify(pendingImages));
    ...
    public async addPendingSyncData(data: any) {
        try {
            ...
            await AsyncStorage.setItem('pendingSyncData', JSON.stringify(pendingData));
    ...
    private async removePendingImage(imageId: string) {
        try {
            ...
            await AsyncStorage.setItem('pendingUploadImages', JSON.stringify(filteredImages));
        } catch (error) {
    ...
    private async removePendingData(dataId: string) {
        try {
            ...
            await AsyncStorage.setItem('pendingSyncData', JSON.stringify(filteredData));
        } catch (error) {
    ...
    private async generateOpenaiStyle(imageUri: string): Promise<void> {
        const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/apple/ai_style`, {
            method: 'POST',
            body: imageUri,
        });
        if (!response.ok) {
