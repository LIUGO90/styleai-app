...
class AsyncAIRequestService {
  private static instance: AsyncAIRequestService;
  private requestQueue: RequestTask[] = [];
  private isProcessing = false;
  private maxConcurrentRequests = 2;
  private activeRequests = 0;

  static getInstance(): AsyncAIRequestService {
    if (!AsyncAIRequestService.instance) {
      AsyncAIRequestService.instance = new AsyncAIRequestService();
    }
    return AsyncAIRequestService.instance;
  }
...
export const asyncAIRequestService = AsyncAIRequestService.getInstance();
